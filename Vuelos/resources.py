...
# Clase que sevirá para mostrar los resultados de todos los registros de Pasajeros
...
# Clase que sevirá para mostrar los resultados de todos los registros de Aerolineas
...
class VuelosRecurso(ModelResource):
    ...
    # Full = True, nos trae los datos, si queremos que nos traiga la url del pasajero o vuelo
    # lo ponemos en False
    Pasajero_vuelo = fields.ForeignKey(PasajerosRecurso, attribute='Pasajero_vuelo', null=True, full=True)
    Linea_aerea = fields.ForeignKey(AerolineasRecurso, attribute='Linea_aerea', null=True, full=True)
    class Meta:
        queryset = Vuelos.objects.all()
        resource_name = 'vuelos'


# Clase que sevirá para mostrar los resultados de todos los registros de Vuelos con pasajeros
class VuelosPasajerosRecurso(ModelResource):
    Pasajero_vuelo = fields.ForeignKey(PasajerosRecurso, attribute='Pasajero_vuelo', null=True, full=True)
    class Meta:
        queryset = Vuelos.objects.all().only("Viaje", "Clase", "Precio", "Ruta", "Pasajero_vuelo")
